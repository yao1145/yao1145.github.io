...
test('fog makes new tracking bullets straight while preserving their aimed direction and slowing them once', () => {
    ...
    assert.ok(Math.abs(speedOf(bullet) - CONFIG.enemyBulletSpeed * 0.8) < 1e-9);
...
test('fog slows every projectile pattern and never applies its multiplier twice', () => {
    ...
    for (const bullet of bullets) {
        ...
        if (bullet.isWave) {
            ...
            assert.ok(Math.abs((bullet.baseVx + waveValue) * 0.8 - bullet.vx) < 1e-9);
        } else {
            assert.equal(bullet.baseVx * 0.8, bullet.vx);
        }
        assert.equal(bullet.baseVy * 0.8, bullet.vy);
    }
    ...
    const expectedWaveVx = (wave.baseVx + Math.sin(wave.waveOffset) * wave.waveAmplitude) * 0.8;
    assert.ok(Math.abs(wave.vx - expectedWaveVx) < 1e-9);
    assert.equal(wave.vy, wave.baseVy * 0.8);
});
...
test('fog boundary warning is set once when a bullet enters the 40px pre-line band', () => {
    ...
    assert.equal(bullet.fogWarningUntil, 1250);
